...
function Cart() {
    ...
    const buyProductMuatation = useMutation({
    ...
    const deletePurchaseMuatation = useMutation({
    ...
    const purchaseIdLocation = (location.state as { purchaseId: string } | null)?.purchaseId;
    const purchasesCart = purchasesCartData?.data.data;
    ...
    const handleQuantity = (purchaseIndex: number, value: number, enable: boolean) => {
        if (enable) {
            ...
            setExtendedPurchases(
                produce((draf) => {
                    draf[purchaseIndex].checked = true;
                }),
            );
    ...
    const handleDelete = (purchaseIndex: number) => () => {
        ...
        deletePurchaseMuatation.mutate([purchaseId]);
    };

    const handleDeletePurchases = () => {
        const purchaseIds = checkedPurchases.map((purchase) => purchase._id);
        deletePurchaseMuatation.mutate(purchaseIds);
    };

    const handeleBuyPurchase = () => {
        if (checkedPurchasesCount > 0) {
            const body = checkedPurchases.map((purchase) => ({
                product_id: purchase.product._id,
                buy_count: purchase.buy_count,
            }));
            buyProductMuatation.mutate(body);
        }
    };
        ...
        setExtendedPurchases((prev) => {
            ...
            return (
                purchasesCart?.map((purchase) => {
                    const isPurchaseFromLocation = purchaseIdLocation === purchase._id;
                    return {
                        ...purchase,
                        disable: false,
    ...
    }, [purchasesCart, purchaseIdLocation]);

    //xoa state purchaseid location khi refresh page
    useEffect(() => {
        return () => {
                    ...
                    <>
                                                            ...
                                                            onFocusOut={(value) =>
                                                                handleQuantity(
                                                                    ...
                                                                    value >= 1 &&
                                                                        value <= purchase.product.quantity &&
                                                                        value !==
                                                                            (purchasesCart as Purchase[])[index]
                                                                                .buy_count,
                                                                )
                                                            }
                                                        />
                        ...
                        <div className="sticky bottom-0 z-10 mt-8 flex flex-col rounded-sm border border-gray-100 bg-white p-5 shadow sm:flex-row sm:items-center">
                            ...
                            <div className="mt-5 flex flex-col sm:ml-auto sm:mt-0 sm:flex-row sm:items-center">
                                ...
                                <Button
                                    disabled={checkedPurchasesCount === 0 || buyProductMuatation.isLoading}
                                    onClick={handeleBuyPurchase}
